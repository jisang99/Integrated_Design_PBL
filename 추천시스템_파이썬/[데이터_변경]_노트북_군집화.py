...
# '스펙' 열을 생성하여 가격을 제외한 문자열 데이터 생성 (가격으로 군집을 이미 한 상태이기 때문에)
...
index = int(input("유저가 본 인덱스 번호 입력 : "))
user_category_1 = data.loc[[index],:]

user_1 = make_user_embedding(user_category_1.index.values.tolist(), data_sepc_content, model)

result, result_index, cos = get_recommened_contents(user_1, data_sepc_content, model)
user1_result = pd.DataFrame(result.loc[:, ['이름','가격', '스펙']])
